...
def get_preface_predict_cmd(
    singularity_exe: str, out_dir: Path, model_dir: str, sample_id: str
) -> str:
...
def get_preface_model_cmd(
    singularity_exe: str, config_path: Path, model_dir: str, settings: str,
) -> str:
    """Create command for doing fetal fraction estimation using Preface"""
    model_cmd = (
        f"singularity exec {singularity_exe} Rscript /bin/PREFACE-0.1.1/PREFACE.R train --config "
