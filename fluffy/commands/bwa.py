...
def get_align_command(
    ...
    cmd = (
        ...
        f"singularity exec {singularity_exe} cat {fastq} | singularity exec {singularity_exe} fastqc stdin:{sample_id}{read} -d {tmp_dir} -o {out_dir}/{sample_id}"
    )
    return cmd


def get_samse_command(
    singularity_exe: str, reference: str, fastq: str, out_prefix: str
) -> str:
    """create a command for running bwa samsw"""
    cmd = f"singularity exec {singularity_exe} bwa samse -n -1 {reference} {out_prefix}.sai {fastq}"
    return cmd


def get_sampe_command(
    singularity_exe: str, reference: str, fastq1: str, fastq2: str, out_prefix: str
) -> str:
    """create a command for running bwa sampe"""
    cmd = (
        f"singularity exec {singularity_exe} bwa sampe -n -1 {reference} "
...
def get_bamsormadup_command(
    singularity_exe: str, tmp_dir: str, sample_id: str, out_prefix: str
) -> str:
    """Create a command for running bamorsamdup"""
    cmd = (
        f"singularity exec {singularity_exe} bamsormadup inputformat=sam threads=16 SO=coordinate "
